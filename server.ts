...
app.post("/api/chat-messages", (req, res) => {
  try {
    const { senderName, avatarUrl, text, mediaType, mediaUrl, role, recipient } = req.body;
    ...
    if (newMessage.role === "student" && newMessage.text) {
      ...
      const directDesc = newMessage.recipient ? `(Shaxsiy xabar to ${newMessage.recipient})` : "";
      const notification = `💬 *Darslik Chatda Yangi Xabar!* ${directDesc}\n👤 O'quvchi: *${newMessage.senderName}*\n📝 Xabar: "${contentDesc}"`;
...
// GET All Registered Users or Search
app.get("/api/users/search", (req, res) => {
  try {
    ...
    const allUsers = Object.values(db.users || {}).map(u => ({
      username: u.username,
      userStreak: u.userStreak || 1,
      wordsLearnedCount: u.wordsLearnedCount || 0,
