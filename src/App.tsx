...
export default function App() {
  ...
  const [studentName, setStudentName] = useState<string>(() => {
    const saved = localStorage.getItem("oxford_student_name");
    if (saved) return saved;
    const randomSuffix = Math.floor(100 + Math.random() * 900);
    const defaultName = `O'quvchi_${randomSuffix}`;
    localStorage.setItem("oxford_student_name", defaultName);
    return defaultName;
  });
  const [userHasEntered, setUserHasEntered] = useState<boolean>(true);
  ...
  const handleLogOutPortal = () => {
    const randomSuffix = Math.floor(100 + Math.random() * 900);
    const newName = `O'quvchi_${randomSuffix}`;
    ...
    // Clear stored stats & set new student name
    localStorage.setItem("oxford_student_name", newName);
    ...
    speakWord("Natijalaringiz muvaffaqiyatli tozalandi va yangi profil berildi.");
  };
