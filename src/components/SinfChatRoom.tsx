...
export function SinfChatRoom({
  ...
  // Direct Message and Account Search states
  ...
  // Load and search registered accounts in school
  const fetchRegisteredUsers = async (query = "") => {
    try {
  ...
  useEffect(() => {
    if (sidebarTab === "users") {
      fetchRegisteredUsers(userSearchQuery);
    }
  ...
  // Final filter for Active Chat Mode (Group vs Direct Message)
  const currentChatMessages = filteredMessages.filter(msg => {
    const msgRecipient = (msg as any).recipient;
    if (activeRecipient) {
      // In private chat with activeRecipient:
      // Show messages sent by activeName to activeRecipient
      // OR sent by activeRecipient to activeName
      return (
        (msg.senderName === activeName && msgRecipient === activeRecipient) ||
        (msg.senderName === activeRecipient && msgRecipient === activeName)
      );
    } else {
      // Group chat mode:
      // Show only public messages (no recipient)
      return !msgRecipient;
    }
  });
        ...
        {sidebarTab === "users" && (
          <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-5 rounded-3xl shadow-xs space-y-4 flex flex-col flex-1 min-h-[420px]">
            ...
            <div className="space-y-3">
              {/* general group chat selection */}
              <button
                onClick={() => {
                  setActiveRecipient(null);
                  playSfx("received");
                }}
                className={`w-full text-left p-2.5 rounded-2xl border flex items-center gap-2.5 transition-all cursor-pointer ${
                  activeRecipient === null
                    ? "bg-sky-50 border-sky-300 dark:bg-slate-800 dark:border-sky-505 shadow-xs"
                    : "bg-transparent border-slate-100 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-900"
                }`}
              >
                <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-sky-400 to-indigo-600 text-white font-black text-xs flex items-center justify-center shadow-inner shrink-0 select-none">
                  👥
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-sky-500 dark:text-sky-400 font-bold">Asosiy guruh kanali</p>
                </div>
                <span className="w-1.5 h-1.5 bg-green-500 rounded-full shrink-0 animate-pulse" />
              </button>

              {/* registered friends searcher */}
              <div className="space-y-1.5 pt-1.5 border-t border-slate-100 dark:border-slate-800">
                <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500 block">
                  🔎 Sinfdoshlarni qidirish (Akkauntlar):
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="E.g., oquvchi '127' yozing..."
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 pl-8 pr-3 py-2 text-xs font-extrabold rounded-xl outline-none focus:ring-1 focus:ring-sky-505 text-slate-800 dark:text-white placeholder-slate-400"
                  />
                  {userSearchQuery && (
                    <button
                      onClick={() => setUserSearchQuery("")}
                      className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 border-0 bg-transparent text-xs font-black cursor-pointer"
                    >
                      ×
                    </button>
                  )}
            ...
            <div className="pb-1 border-b border-slate-100 dark:border-slate-800 pt-1.5 flex items-center justify-between">
              <h4 className="text-xs font-black uppercase tracking-wider text-[#002147] dark:text-slate-250">
                {userSearchQuery ? "Qidiruv Natijalari" : "Barcha Akkauntlar"}
              </h4>
              <span className="text-[9px] bg-slate-100 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded font-black">
                {registeredUsers.length} ta
              </span>
            </div>

            {/* List scrollbar */}
            <div className="space-y-2 flex-1 overflow-y-auto max-h-[260px] lg:max-h-none pr-1">
              {registeredUsers.length > 0 ? (
                registeredUsers.map((usr, idx) => {
                  const isYou = usr.username === studentName || usr.username === activeName;
                  const isOnline = onlineList.includes(usr.username);
                  const isSelected = activeRecipient === usr.username;
                  
                  // Check for unread message from this specific user
                  const hasUnread = messages.some(m => m.senderName === usr.username && (m as any).recipient === activeName && activeRecipient !== usr.username);

                  // Stable unique visual color seed based on name length/hash
                  const hash = usr.username.split("").reduce((acc: number, char: string) => acc + char.charCodeAt(0), 0);
                  const gradients = [
                    "from-blue-500 to-indigo-600",
                    "from-emerald-400 to-teal-600",
                    "from-purple-505 to-pink-600",
                  ...
                  return (
                    <button
                      key={usr.username}
                      onClick={() => {
                        setActiveRecipient(usr.username);
                        playSfx("received");
                      }}
                      className={`w-full text-left flex items-center gap-2 p-2 rounded-xl border transition-all cursor-pointer ${
                        isSelected 
                          ? "bg-sky-50 border-sky-300 dark:bg-slate-800 dark:border-sky-505" 
                          : "bg-transparent border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-950"
                      }`}
                    >
                      <div className="relative shrink-0 select-none hidden sm:block">
                        {isYou ? (
                          ...
                          <div className={`w-7 h-7 rounded-full bg-gradient-to-tr ${grad} text-white font-black text-[10px] flex items-center justify-center uppercase shadow-xs`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-black text-slate-800 dark:text-slate-200 truncate flex items-center gap-1">
                          <span>{usr.username}</span>
                          {isYou && <span className="text-sky-500 text-[9px]">(Siz)</span>}
                        </p>
                        <p className="text-[9.5px] font-mono text-slate-400 mt-0.5">
                          🔥 {usr.userStreak} kun | 📖 {usr.wordsLearnedCount} so'z
                        </p>
                      </div>
                      {hasUnread && (
                        <span className="shrink-0 bg-red-500 text-white font-black text-[8px] px-1.5 py-0.5 rounded-full uppercase animate-pulse">
                          yangi
                        </span>
                      )}
                    </button>
                ...
                <div className="text-center py-6 text-slate-400 text-xs font-medium">
                  {userSearchQuery ? "Bunday foydalanuvchi topilmadi!" : "Barcha ro'yxatdan o'tgan talabalar yuklanmoqda..."}
                </div>
              )}
            </div>

            {/* Total metric stats info */}
            <div className="p-3 bg-amber-50/50 dark:bg-amber-950/10 border border-amber-200/50 rounded-2xl text-[10.5px] text-[#002147]/80 dark:text-amber-305 flex items-start gap-1.5 leading-relaxed shrink-0">
              <span>🎁</span>
              <span>
                Foydalanuvchini tanlab, u bilan guruhdan tashqari shaxsiy ravishda xabar yozishishingiz mumkin!
              </span>
            </div>
          </div>
        )}
        ...
        <div className="bg-white dark:bg-[#18202F] border-b border-slate-150 dark:border-slate-800/80 p-3.5 md:p-4 shrink-0 flex items-center justify-between z-10 relative">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="relative shrink-0 select-none">
              {activeRecipient ? (
                <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-amber-400 to-orange-500 text-white font-black text-sm flex items-center justify-center shadow-inner tracking-wider select-none uppercase">
                  {activeRecipient.substring(0, 2)}
                </div>
              ) : (
              ...
              <span className={`absolute bottom-0 right-0 w-2.5 h-2.5 border-2 border-white rounded-full ${
                activeRecipient 
                  ? (onlineList.includes(activeRecipient) ? "bg-emerald-500 animate-pulse" : "bg-slate-400")
                  : "bg-emerald-500 animate-pulse"
              }`} />
            </div>

            <div className="min-w-0">
              <h3 className="text-xs md:text-sm font-black text-slate-800 dark:text-slate-100 truncate flex items-center gap-1.5">
                {activeRecipient ? (
                  <>
                    <span>👤 {activeRecipient}</span>
                    <span className="text-[10px] bg-slate-100 dark:bg-slate-800 text-sky-700 dark:text-sky-300 px-2 py-0.5 rounded-full font-black">Shaxsiy (DM)</span>
                  </>
                ) : (
                  "Interactive Oxford Discover Chat 🎓"
                )}
              </h3>
              <p className="text-[10px] text-sky-500 dark:text-sky-400 font-bold flex items-center gap-1.5 mt-0.5 font-sans">
                {activeRecipient ? (
                  <span>{onlineList.includes(activeRecipient) ? "🟢 Hozir online" : "⚪ Faol emas"}</span>
                ) : (
                  <span>{onlineCount} active sinfdoshlar online</span>
                )}
                <span>·</span>
                <span className="bg-amber-100 text-amber-800 dark:bg-amber-950/40 dark:text-amber-300 px-1 py-0.2 rounded-sm text-[8px] font-black uppercase font-mono">
                  Telegram mode
                </span>
              </p>
            </div>
          ...
          {/* Inline message search option or general buttons */}
          <div className="flex items-center gap-2">
            {activeRecipient && (
              <button
                onClick={() => setActiveRecipient(null)}
                className="px-3 py-1.5 bg-sky-500 hover:bg-sky-600 text-white text-xs font-black rounded-xl cursor-pointer transition-colors flex items-center gap-1 border-0 shadow-xs uppercase tracking-wider"
              >
                👥 Guruh suhbati
              </button>
            )}
            ...
            <div className="relative hidden sm:block">
              ...
              <input
                ...
                className="bg-slate-100 dark:bg-slate-900 border-0 rounded-2xl pl-8 pr-3 py-1.5 text-[10.5px] font-bold outline-none placeholder-slate-400 text-slate-700 dark:text-slate-200 focus:ring-1 focus:ring-sky-505 w-36 md:w-44 transition-all"
              />
            </div>

            <button
              onClick={() => fetchMessages(true)}
              className="p-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer transition-colors border-0"
